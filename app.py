...
PAGINAS = ["Dashboard", "Añadir Gasto", "Transacciones", "Importar", "Categorías", "🔌 Coche Eléctrico", "🤖 Asistente IA", "Sincronización", "Configuración"]
...
def mostrar_dashboard():
    ...
    Si USE_NEW_DASHBOARD está activo, usa la versión v2 con nuevo diseño.
    Si está desactivo, usa la versión v1 original (fallback).
    """
    # Feature flag: Usar versión v2 o v1
    if is_enabled('USE_NEW_DASHBOARD'):
        # Llamar a versión v2 con parámetros necesarios
...
def mostrar_transacciones():
    """
    Página de transacciones con soporte para v1 (viejo) y v2 (nuevo diseño).

    - v1: Diseño actual con st.title y st.data_editor clásico
    - v2: Nuevo diseño con render_table_layout, render_transaction_table y form_card
    """
    from utils.feature_flags import is_enabled

    if is_enabled('USE_NEW_TRANSACCIONES'):
        mostrar_transacciones_v2()
    else:
        mostrar_transacciones_v1()


def mostrar_transacciones_v1():
    """Versión original de la página de transacciones (preservada para rollback)."""
...
def mostrar_transacciones_v2():
    """
    Versión v2 de la página de transacciones con el nuevo sistema de diseño.

    Características:
    - render_table_layout() para estructura general
    - Filtros en sidebar (mes, año, categorías)
    - Formulario para añadir nuevas transacciones (colapsable)
    - Editor inline con st.data_editor para modificar transacciones existentes
    - Estadísticas de resumen (totales, gastos, ingresos, balance)
    - Preserva toda la funcionalidad CRUD original
    """
    from utils.components import (
        render_table_layout,
        page_section,
    ...
    def render_filtros_sidebar():
        """Renderiza los filtros de período y categoría en el sidebar."""
        st.markdown("### Filtros")
        st.markdown("---")
        ...
        nombre_mes_seleccionado = st.selectbox(
            "Mes",
            meses_disponibles,
            index=index_mes,
            key="trans_v2_mes"
        ...
        categorias_seleccionadas = st.multiselect(
            ...
            ["Todas"] + categorias_db,
            default=["Todas"],
            key="trans_v2_categorias"
        )

        return mes, año, categorias_seleccionadas

    # === CONTENIDO PRINCIPAL ===
    def render_contenido_transacciones():
        """Renderiza el contenido principal de la página."""
        # Obtener filtros del sidebar
        mes = st.session_state.mes_seleccionado
        año = st.session_state.año_seleccionado
        categorias_seleccionadas = st.session_state.get('trans_v2_categorias', ['Todas'])

        # === SECCIÓN: AÑADIR NUEVA TRANSACCIÓN ===
        with page_section(
            ...
            with st.form("form_nueva_transaccion_v2"):
                col1, col2 = st.columns(2)
                ...
                with col1:
                    fecha = st.date_input(
                        "Fecha",
                        value=datetime.date.today(),
                        key="nueva_trans_fecha"
                    )

                    concepto = st.text_input(
                        "Concepto",
                        placeholder="Ej: Compra en supermercado",
                        key="nueva_trans_concepto"
                    )
                ...
                with col2:
                    categoria = st.selectbox(
                        "Categoría",
                        ["FIJOS", "DISFRUTE", "EXTRAORDINARIOS", "REEMBOLSO", "INGRESO", "SIN_CLASIFICAR"],
                        key="nueva_trans_categoria"
                    )

                    tipo = st.selectbox(
                        "Tipo",
                        ["GASTO", "INGRESO"],
                        key="nueva_trans_tipo"
                    )

                    notas = st.text_area(
                        "Notas (opcional)",
                        placeholder="Notas adicionales...",
                        key="nueva_trans_notas",
                        height=100
                    )
                ...
                if submitted:
                    ...
                    if not concepto or not concepto.strip():
                        ...
                        try:
                            ...
                            st.error(f"Error al guardar la transacción: {e}")
            ...
            with st.spinner("Cargando transacciones..."):
                ...
                if not df_original.empty:
                    ...
                    if df_filtrado.empty:
                        st.info("No hay transacciones para las categorías seleccionadas.")
                        return

                    # Convertir fecha a datetime para el editor
                    df_filtrado['fecha'] = pd.to_datetime(df_filtrado['fecha'])

                    # === MODO EDICIÓN CON DATA_EDITOR ===
                    st.info("💡 **Edición inline**: Haz doble clic en cualquier celda para editarla. Los cambios se guardan al hacer clic en 'Guardar Cambios'.")
                    ...
                    configuracion_columnas = {
                        ...
                        "categoria": st.column_config.SelectboxColumn(
                            "Categoría",
                            options=["FIJOS", "DISFRUTE", "EXTRAORDINARIOS", "REEMBOLSO", "INGRESO", "SIN_CLASIFICAR"],
                            width="medium"
                        ),
                    ...
                    df_editado = st.data_editor(
                        ...
                        hide_index=True,
                        use_container_width=True,
                        key="data_editor_transacciones_v2"
                    )
                    ...
                    try:
                        if not df_editado.equals(df_a_mostrar):
                            hay_cambios = True
                            st.warning("⚠️ **Hay cambios sin guardar**. Haz clic en 'Guardar Cambios' para persistirlos.", icon="⚠️")
                    except:
                        pass
                    ...
                    with col_save3:
                        if st.button("💾 Guardar Cambios", type="primary" if hay_cambios else "secondary", key="btn_guardar_v2"):
                            # Validar que existe columna 'id'
                            if 'id' not in df_editado.columns:
                                st.error("❌ Error: La columna 'id' es necesaria para guardar cambios. Inclúyela en las columnas visibles.")
                            else:
                                # Reconstruir DataFrame con ediciones
                                df_con_ediciones = df_filtrado.copy()
                                for col in df_editado.columns:
                                    if col in df_con_ediciones.columns:
                                        df_con_ediciones[col] = df_editado[col]
                    ...
                    with col_stat2:
                        total_gastos = df_filtrado[df_filtrado['tipo'] == 'GASTO']['importe'].sum() if 'tipo' in df_filtrado.columns else 0
                        st.metric("Total Gastos", f"{total_gastos:,.2f} €")

                    with col_stat3:
                        total_ingresos = df_filtrado[df_filtrado['tipo'] == 'INGRESO']['importe'].sum() if 'tipo' in df_filtrado.columns else 0
                    ...
                    st.info("No se encontraron transacciones para el período seleccionado.")
                    st.markdown("---")
                    st.markdown("💡 **Sugerencia**: Añade tu primera transacción usando el formulario de arriba.")

    # === RENDERIZAR LAYOUT COMPLETO ===
    render_table_layout(
        content_fn=render_contenido_transacciones,
        title="Transacciones",
        description="Gestiona y visualiza todas tus transacciones financieras",
        icon="💸",
        show_filters=True,
        filters_fn=render_filtros_sidebar,
        show_export_button=False  # Ya tenemos exportación en render_data_table
    )
...
def mostrar_importar_v1():
    """Versión original de importar (preservada para rollback)"""
...
def mostrar_importar_v2():
    """
    Versión v2 de importar con nuevo sistema de diseño.

    Características:
    - Usa render_form_layout() para layout consistente
    - form_card para uploader y opciones
    - render_data_table() para preview
    - show_form_feedback() para mensajes
    - page_section() para organizar pasos
    - Progress bar durante importación
    """

    def render_contenido():
        """Función de contenido para render_form_layout"""

        # === PASO 1: UPLOADER DE ARCHIVO ===
        with page_section(
            title="1. Seleccionar Archivo",
            description="Sube tu archivo Excel (.xlsx) con los movimientos bancarios",
            icon="📂"
        ):
            def render_uploader():
                st.markdown(f"""
                ...
                ">
                    ...
                    <ul style="margin: {Spacing.XS} 0; padding-left: {Spacing.LG};">
                        <li>Formato: Excel (.xlsx)</li>
                        <li>Columnas esperadas: Fecha, Concepto, Importe</li>
        ...
        # Si no hay archivo, detener aquí
        if 'file_uploader_v2' not in st.session_state or st.session_state.file_uploader_v2 is None:
            return

        uploaded_file = st.session_state.file_uploader_v2
        ...
        if 'import_data' not in st.session_state or st.session_state.get('uploaded_filename') != uploaded_file.name:
            with st.spinner("Procesando archivo Excel..."):
                ...
                # Leer Excel
                transacciones, stats = excel_reader.leer_excel(uploaded_file)
                progress_bar.progress(30, text="Analizando transacciones...")
                ...
                progress_bar.progress(60, text="Detectando duplicados...")
        ...
        # === PASO 2: RESUMEN Y ESTADÍSTICAS ===
        if "error" in stats:
            show_form_feedback(
                f"Error al procesar el archivo: {stats['error']}",
                type="error"
            )
            return

        with page_section(
            title="2. Resumen del Archivo",
            description="Estadísticas de las transacciones encontradas",
            icon="📊"
        ):
            col1, col2, col3 = st.columns(3)
            ...
            with col1:
                ...
                ">
                    <div style="font-size: {Typography.TEXT_3XL}; font-weight: {Typography.WEIGHT_BOLD}; color: {Colors.PRIMARY};">
                        {stats.get('total_sheets_processed', 0)}
                    </div>
                    <div style="font-size: {Typography.TEXT_SM}; color: {Colors.GRAY_600}; margin-top: {Spacing.XS};">
                        Hojas Procesadas
                    </div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with col2:
                ...
                <div style="
                    padding: {Spacing.LG};
                    background: {Colors.SUCCESS_ULTRA_LIGHT};
                    border-radius: {BorderRadius.BASE};
                    text-align: center;
                    border: 2px solid {Colors.SUCCESS};
                ">
                    <div style="font-size: {Typography.TEXT_3XL}; font-weight: {Typography.WEIGHT_BOLD}; color: {Colors.SUCCESS};">
                        {len(nuevas)}
                    </div>
                    <div style="font-size: {Typography.TEXT_SM}; color: {Colors.GRAY_600}; margin-top: {Spacing.XS};">
            ...
            with col3:
                ...
                ">
                    <div style="font-size: {Typography.TEXT_3XL}; font-weight: {Typography.WEIGHT_BOLD}; color: {Colors.WARNING};">
                        {len(duplicadas)}
                    </div>
                    <div style="font-size: {Typography.TEXT_SM}; color: {Colors.GRAY_600}; margin-top: {Spacing.XS};">
        ...
        if nuevas:
            with page_section(
                title="3. Preview de Transacciones Nuevas",
                description="Primeras 10 transacciones que se importarán",
                icon="👀"
            ):
                df_nuevas = pd.DataFrame(nuevas).head(10)
                render_data_table(
                    df_nuevas,
                    searchable=False,
                    exportable=False,
        ...
        if duplicadas:
            with page_section(
                title="4. Transacciones Duplicadas",
                description=f"Se detectaron {len(duplicadas)} transacciones que ya existen en la base de datos",
                icon="⚠️"
            ):
                show_form_feedback(
                    f"Atención: Se encontraron {len(duplicadas)} transacciones con la misma fecha e importe",
                    type="warning"
                )
                ...
                render_data_table(
                    df_duplicadas,
                    title="Transacciones Duplicadas Detectadas",
                    searchable=False,
                    exportable=False,
                    pagination=False,
                    height=250,
                    key="preview_duplicadas",
                    currency_columns=['importe'],
                    date_columns=['fecha']
                )
                ...
                accion_duplicados = st.radio(
                    "¿Qué deseas hacer con las transacciones duplicadas?",
                    options=['Omitir duplicados (Recomendado)', 'Importar todo (creará duplicados)'],
                    key='accion_duplicados_v2',
                    help="Recomendamos omitir duplicados para evitar registros repetidos"
                )
        else:
            accion_duplicados = 'Omitir duplicados (Recomendado)'

        # === PASO 5: CONFIRMACIÓN E IMPORTACIÓN ===
        if transacciones:
            with page_section(
                title="5. Confirmar Importación",
                description="Revisa el resumen y confirma para importar las transacciones",
                icon="✅"
            ):
                # Calcular transacciones a importar
                if accion_duplicados == 'Omitir duplicados (Recomendado)':
                    transacciones_a_importar = nuevas
                    mensaje_accion = f"Se importarán {len(nuevas)} transacciones nuevas"
                    if duplicadas:
                        mensaje_accion += f" (se omitirán {len(duplicadas)} duplicados)"
                else:
                    transacciones_a_importar = transacciones
                    mensaje_accion = f"Se importarán TODAS las {len(transacciones)} transacciones (incluyendo duplicados)"

                st.info(mensaje_accion)

                # Botón de importar
                col_btn1, col_btn2 = st.columns([1, 3])

                with col_btn1:
                    if st.button("✅ Confirmar e Importar", type="primary", use_container_width=True, key="btn_confirmar_v2"):
                        if not transacciones_a_importar:
                            show_form_feedback("No hay transacciones nuevas para importar", type="info")
                        else:
                            # Importación con progress bar
                            progress_text = st.empty()
                            progress_bar = st.progress(0)

                            count = 0
                            total = len(transacciones_a_importar)

                            for idx, t in enumerate(transacciones_a_importar):
                                # Actualizar progreso
                                progress = int((idx + 1) / total * 100)
                                progress_bar.progress(progress)
                                progress_text.text(f"Importando transacción {idx + 1} de {total}...")
                            ...
                            # Éxito
                            show_form_feedback(
                                f"Importación completada: {count} transacciones guardadas correctamente",
                                type="success"
                            )
                            st.balloons()

                            # Limpiar estado
                            for key in ['import_data', 'import_stats', 'uploaded_filename', 'nuevas_transacciones', 'transacciones_duplicadas', 'file_uploader_v2']:
                                if key in st.session_state:
                                    del st.session_state[key]
                ...
                with col_btn2:
                    if st.button("🔄 Cargar Otro Archivo", use_container_width=True, key="btn_reset_v2"):
                        # Limpiar estado
                        for key in ['import_data', 'import_stats', 'uploaded_filename', 'nuevas_transacciones', 'transacciones_duplicadas', 'file_uploader_v2']:
                            if key in st.session_state:
                                del st.session_state[key]
    ...
    # Renderizar con layout de formulario
    render_form_layout(
        content_fn=render_contenido,
        title="Importar desde Excel",
        description="Sube tu archivo Excel con los movimientos bancarios para procesarlos e importarlos a la base de datos",
        icon="📥"
    )

def mostrar_categorias():
    """Wrapper que delega a v1 o v2 según feature flag"""
    if is_enabled('USE_NEW_CATEGORIAS'):
        mostrar_categorias_v2()
        return
    mostrar_categorias_v1()

def mostrar_categorias_v1():
...
def mostrar_categorias_v2():
    """
    Versión 2 de Categorías con nuevo sistema de diseño.
    Usa render_table_layout + render_data_table + form_card
    """
    from utils.components.page_layout import render_table_layout, page_section
    from utils.components.form_card import render_form_card, show_form_feedback
    from utils.components.data_table import render_data_table

    def contenido_principal():
        """Contenido principal de la página de categorías"""

        # ========== SECCIÓN: REGLAS ACTUALES ==========
        with page_section(
            title="Reglas Actuales",
            description="Lista de reglas configuradas para clasificación automática",
            icon="📋"
        ):
            ...
            try:
                ...
                if reglas_actuales:
                    # Convertir reglas a DataFrame para mostrar en tabla
                    reglas_data = []
                    for i, regla in enumerate(reglas_actuales):
                        patron = regla.get("patron", "")
                        importes = regla.get("importes_exactos", [])
                        categoria = regla.get("categoria", "N/A")
                        tipo = regla.get("tipo", "N/A")

                        regla_display = {
                            'id': i,
                            'Patrón': patron if patron else f"Importes: {', '.join(map(str, importes))}",
                            'Categoría': categoria,
                            'Tipo': tipo,
                            'Importes Exactos': ', '.join(map(str, importes)) if importes else '-'
                        }
                        reglas_data.append(regla_display)

                    df_reglas = pd.DataFrame(reglas_data)

                    # Renderizar tabla con componente nuevo
                    render_data_table(
                        df_reglas,
                        columns=['Patrón', 'Categoría', 'Tipo', 'Importes Exactos'],
                        searchable=True,
                        exportable=False,
                        pagination=False,
                        key="tabla_reglas_categorias"
                    )

                    # Acciones por regla (usar expanders para editar/eliminar)
                    st.markdown(f"<div style='height: {Spacing.MD}'></div>", unsafe_allow_html=True)
                    st.caption("**Acciones:** Expande cada regla para ver opciones de editar o eliminar")
                        ...
                        with st.expander(titulo_expander):
                            ...
                            with col1:
                                if st.button("Editar", key=f"edit_{i}"):
                                    st.info("Funcionalidad de edición en desarrollo. Por ahora, elimina la regla y créala de nuevo.")

                            with col2:
                                if st.button("Eliminar", key=f"del_{i}", help="Eliminar esta regla permanentemente"):
                                    if categorizer.eliminar_regla(patron):
                                        st.success(f"Regla para '{patron}' eliminada.")
                                        st.rerun()
                                    else:
                    ...
                    st.info("No hay reglas configuradas aún. Crea tu primera regla abajo.")
        ...
        # ========== SECCIÓN: AÑADIR NUEVA REGLA ==========
        with page_section(
            title="Añadir Nueva Regla",
            description="Crea una regla para clasificar automáticamente transacciones",
            icon="➕",
            margin_top=Spacing.XXL
        ):
            def formulario_nueva_regla():
                """Contenido del formulario para nueva regla"""
                nuevo_patron = st.text_input(
                    "Patrón de texto",
                    placeholder="Ej: MERCADONA, AMAZON, etc.",
                    help="Puede ser una expresión regular",
                    key="nuevo_patron_cat"
                )

                importes_str = st.text_input(
                    "Importes exactos (opcional)",
                    placeholder="Ej: 500, 275.50",
                    help="Separados por coma. Útil para identificar pagos recurrentes fijos",
                    key="importes_str_cat"
                )
                    ...
                    if nuevo_patron or importes_str:
                        ...
                        if importes_str:
                            try:
                                ...
                                show_form_feedback(
                                    "El formato de los importes no es válido. Usa números separados por comas (ej: 500, 275.5).",
                                    type="error"
                                )
                                return
                        ...
                        if categorizer.guardar_regla(nuevo_patron, nueva_categoria, nuevo_tipo, importes_exactos):
                            show_form_feedback(
                                f"Regla '{nuevo_patron}' → '{nueva_categoria}' guardada correctamente",
                                type="success"
                            )
                            st.rerun()
                        else:
                            show_form_feedback(
                                "No se pudo guardar la regla. Quizás el patrón ya existe.",
                                type="error"
                            )
                    else:
                        show_form_feedback(
                            "Debes proporcionar al menos un patrón de texto o uno o más importes exactos.",
                            type="warning"
                        )

            # Renderizar formulario con form_card
            with st.form(key="form_nueva_regla", clear_on_submit=True):
                formulario_nueva_regla()

    # Renderizar página completa con layout
    render_table_layout(
        content_fn=contenido_principal,
        title="Categorías y Reglas de Clasificación",
        description="Gestiona las reglas que se usan para clasificar automáticamente tus transacciones",
        icon="🏷️",
        show_filters=False,
...
def mostrar_configuracion():
    """Wrapper que delega a v1 o v2 según feature flag"""
    if is_enabled('USE_NEW_CONFIGURACION'):
        mostrar_configuracion_v2()
        return
    mostrar_configuracion_v1()

def mostrar_configuracion_v1():
    ...
    # ========== SECCIÓN: UTILIDADES DE MANTENIMIENTO ==========
    st.subheader("🔧 Utilidades de Mantenimiento")

    # Información sobre saldos posteriores
    col_info_saldo, col_boton_recalcular = st.columns([2, 1])

    with col_info_saldo:
        st.info(
            "📊 **Calcular Saldos de Transacciones Manuales**: Calcula el campo `saldo_posterior` **SOLO** "
            "para transacciones manuales (pagos coche eléctrico, etc.) que no tienen saldo.\n\n"
            "⚠️ **IMPORTANTE**: No modifica las transacciones importadas del banco, que ya tienen su saldo real.",
            icon="ℹ️"
        )

        # Mostrar estadísticas de saldos
        conn = db_manager.get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) as total FROM transacciones WHERE saldo_posterior IS NULL")
        sin_saldo = cursor.fetchone()['total']
        cursor.execute("SELECT COUNT(*) as total FROM transacciones")
        total_trans = cursor.fetchone()['total']
        conn.close()

        if sin_saldo > 0:
            st.warning(f"⚠️ Hay **{sin_saldo}** transacciones manuales sin saldo_posterior")
        else:
            st.success(f"✅ Todas las transacciones tienen saldo_posterior ({total_trans} total)")

    with col_boton_recalcular:
        st.markdown("**Filtrar por período (opcional):**")
        ...
        with col_mes:
            mes_filtro = st.selectbox(
                ...
                format_func=lambda x: "Todos" if x is None else f"{x:02d}",
                key="filtro_mes_saldos"
            )
        ...
        with col_año:
            ...
            año_filtro = st.selectbox(
                "Año",
                options=[None] + años_disponibles,
                format_func=lambda x: "Todos" if x is None else str(x),
                key="filtro_año_saldos"
            )

        if st.button("🔄 Calcular Saldos Manuales", type="primary", use_container_width=True):
            filtro_msg = ""
            if mes_filtro and año_filtro:
                filtro_msg = f" de {mes_filtro:02d}/{año_filtro}"
            elif mes_filtro:
                filtro_msg = f" del mes {mes_filtro:02d}"
            elif año_filtro:
                filtro_msg = f" del año {año_filtro}"

            with st.spinner(f"Calculando saldos{filtro_msg}..."):
                num_actualizadas = db_manager.recalcular_saldos_transacciones_manuales(
                    mes=mes_filtro,
                    año=año_filtro
                )

            if num_actualizadas > 0:
                st.success(f"✅ Se calcularon {num_actualizadas} saldos{filtro_msg}")
                st.rerun()
            elif num_actualizadas == 0:
                st.info(f"ℹ️ No hay transacciones manuales sin saldo{filtro_msg}")
            else:
                st.error("❌ Error al calcular los saldos")

    # Botón para mostrar detalles técnicos (expander)
    with st.expander("🔍 Ver transacciones sin saldo (detalles técnicos)"):
        conn = db_manager.get_db_connection()
        cursor = conn.cursor()
        cursor.execute("""
            SELECT fecha, concepto, importe, categoria
            ...
            LIMIT 10
        """)
        trans_sin_saldo = cursor.fetchall()
        conn.close()

        if trans_sin_saldo:
            df_sin_saldo = pd.DataFrame(trans_sin_saldo)
            df_sin_saldo.columns = ['Fecha', 'Concepto', 'Importe (€)', 'Categoría']
            st.dataframe(df_sin_saldo, use_container_width=True)
        else:
            st.info("✅ No hay transacciones sin saldo_posterior")

def mostrar_configuracion_v2():
    """
    Versión 2 de Configuración con nuevo sistema de diseño.
    Usa render_form_layout + form_card + page_section()
    """
    from utils.components.page_layout import render_form_layout, page_section, page_divider
    from utils.components.form_card import render_form_card, show_form_feedback
    from utils.components.data_table import render_data_table
    ...
    def contenido_principal():
        ...
        with page_section(
            title="Saldo Inicial",
            description="El saldo inicial se suma a todas las transacciones para calcular el líquido disponible total",
            icon="💰"
        ):
            config_actual = config_manager.obtener_config()

            def formulario_saldo_inicial():
                """Formulario para configurar el saldo inicial"""
                col1, col2 = st.columns(2)
                ...
                if submitted:
                    if config_manager.actualizar_saldo_inicial(saldo_inicial, fecha_inicial, notas_inicial):
                        show_form_feedback(f"Saldo inicial actualizado: {saldo_inicial:.2f} €", type="success")
                        st.rerun()
                    else:
                        show_form_feedback("Error al guardar el saldo inicial", type="error")

            with st.form("form_saldo_inicial_v2"):
                formulario_saldo_inicial()

        page_divider()

        # ========== SECCIÓN: PRESUPUESTOS MENSUALES ==========
        with page_section(
            title="Presupuestos Mensuales por Categoría",
            description="Define límites mensuales de gasto por categoría para controlar mejor tus finanzas",
            icon="📊"
        ):
            # Mostrar presupuestos actuales
            presupuestos_actuales = db_manager.obtener_presupuestos()
            ...
            if presupuestos_actuales:
                ...
                render_data_table(
                    ...
                    pagination=False,
                    auto_format=False,
                    key="tabla_presupuestos_v2"
                )

                st.markdown(f"<div style='height: {Spacing.MD}'></div>", unsafe_allow_html=True)
                st.caption("**Acciones:** Usa el botón de eliminar junto a cada presupuesto")

                # Botones de eliminar por presupuesto
                for idx, presupuesto in enumerate(presupuestos_actuales):
                    col1, col2, col3 = st.columns([2, 2, 1])
                    ...
                    with col3:
                        if st.button("Eliminar", key=f"del_presupuesto_v2_{idx}", help="Eliminar presupuesto"):
                            if db_manager.eliminar_presupuesto(presupuesto['categoria']):
                                st.success(f"Presupuesto de {presupuesto['categoria']} eliminado")
                                st.rerun()
                            else:
                ...
                page_divider(margin=Spacing.LG)
            ...
            with col2:
                limite_mensual = st.number_input(
                    ...
                    step=10.00,
                    format="%.2f",
                    key="presupuesto_limite_v2",
                    help="Cantidad máxima que quieres gastar en esta categoría cada mes"
                )

            if st.button("Guardar Presupuesto", type="primary", use_container_width=True, key="btn_guardar_presupuesto_v2"):
                try:
                    resultado = db_manager.crear_presupuesto(categoria_presupuesto, limite_mensual)
                    if resultado:
                        st.success(f"Presupuesto para {categoria_presupuesto} guardado: {limite_mensual:.2f} €/mes")
        ...
        with page_section(
            ...
            description="Operaciones avanzadas sobre la base de datos",
            icon="🗄️"
        ):
            col_info, col_reset = st.columns([2, 1])
            ...
            with col_info:
                st.warning(
                    "**Resetear la base de datos eliminará todas las transacciones**, "
                    "pero el saldo inicial se mantendrá guardado y podrás restaurarlo después."
                )

            with col_reset:
                if st.button("Resetear Base de Datos", type="secondary", use_container_width=True, key="reset_db_v2"):
                    confirmacion = st.warning(
                        "¿Estás seguro de que quieres resetear la base de datos? ¡Esta acción borrará todos los datos!"
                    )
                    if confirmacion:
        ...
        # ========== SECCIÓN: UTILIDADES DE MANTENIMIENTO ==========
        with page_section(
            title="Utilidades de Mantenimiento",
            description="Herramientas para mantener la integridad de los datos",
            icon="🔧"
        ):
            col_info_saldo, col_boton_recalcular = st.columns([2, 1])
            ...
            with col_info_saldo:
                st.info(
                    "**Calcular Saldos de Transacciones Manuales**: Calcula el campo `saldo_posterior` **SOLO** "
                    "para transacciones manuales (pagos coche eléctrico, etc.) que no tienen saldo.\n\n"
                    "**IMPORTANTE**: No modifica las transacciones importadas del banco, que ya tienen su saldo real."
                )
                ...
                if sin_saldo > 0:
                    st.warning(f"Hay **{sin_saldo}** transacciones manuales sin saldo_posterior")
                else:
                    st.success(f"Todas las transacciones tienen saldo_posterior ({total_trans} total)")
            ...
            with col_boton_recalcular:
                ...
                with col_año:
                    ...
                    año_filtro = st.selectbox(
                        ...
                        options=[None] + años_disponibles,
                        format_func=lambda x: "Todos" if x is None else str(x),
                        key="filtro_año_saldos_v2"
                    )

                if st.button("Calcular Saldos Manuales", type="primary", use_container_width=True, key="calc_saldos_v2"):
                    filtro_msg = ""
                    if mes_filtro and año_filtro:
                        filtro_msg = f" de {mes_filtro:02d}/{año_filtro}"
                    elif mes_filtro:
                    ...
                    if num_actualizadas > 0:
                        ...
                        st.info(f"No hay transacciones manuales sin saldo{filtro_msg}")
                    else:
                        st.error("Error al calcular los saldos")
    ...
    render_form_layout(
        content_fn=contenido_principal,
        title="Configuración",
        description="Ajusta los parámetros principales de la aplicación",
        icon="⚙️"
    )
...
elif pagina_seleccionada == "🤖 Asistente IA":
    pages_asistente_ia.mostrar_asistente_ia()
